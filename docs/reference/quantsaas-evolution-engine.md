**文件目標:** 將進化系統降維為一個純粹的尋優黑盒。進化引擎不關心具體策略內部結構，只透過抽象介面驅動種群生命週期。具體策略的染色體字段、邊界、適應度語義由策略模塊在各自文件中定義。
...
引擎不關心：策略內部的信號公式、部位三態語義、特定時鐘機制，也不 import 任何具體策略實現。引擎只知道：基因是一個可采樣、可變異、可交叉、可評分的抽象向量。
...
Epoch 啟動時，引擎一次性從資料庫拉取**本 Epoch 所需的全部**歷史 K 線序列用於構造坩堝：**不對全庫長度設「最多十年」等人工上限**；能有多長取決於上游已同步寫入 Postgres 的資料，而下限以**資料源側可拉取到的最早可用 bar** 為準（例如滬深300ETF、黃金ETF 等標的能同步多早就多早，直至 API/上市歷史起點）。在此基礎上再構造以下四個滾動窗口（標識「10y」表示長週期檔，**評分區間取入庫全序列**——自最早可用 bar 至最新 bar，不設人工天數上限）。
...
表中「目標時長」：5y / 2y / 6m 為對齊最新一根 bar 的最近若干日歷日，並在該截取區間之前再預留最多 1200 天 warmup K 線（用於指標預熱，不計入評分區間）。**10y 檔不再單獨截斷為固定天數**，直接使用本 Epoch 傳入的 `bars` 全序列；若序列長於十年，則長週期評分自然覆蓋更長歷史。更短窗口仍使用同一全庫序列中的尾部片段及各自 warm-up 前綴。

**嚴禁未來資料泄露：** 每個窗口的評估區間從 `EvalStartMs` 開始，warmup 資料必須在 `EvalStartMs` 之前。


### 1.2 基因空間窗口

策略透過以下語義向引擎提供基因邊界，引擎不直接讀取字段名：

- **采樣（Sample）：** 從合法空間亂數生成一個基因執行個體
- **修復（Clamp）：** 將越界基因修復到合法范圍（含結構約束修復）
...
當資料庫中存在精英基因時：
- **10%**：從資料庫精英中原樣複製
- **40%**：在精英基礎上施加加性高斯強化變異（初始化路徑固定變異機率 `0.15`、尺度 `1.5`，與代內 `MutationRamp` 無關）
- **50%**：完全亂數生成

當資料庫無精英時：第一個體為產品預設冠軍染色體原樣，其余個體完全亂數生成。

實現約定：索引 0 始終為當前種子冠軍原樣（有 DB 精英則為庫中冠軍染色體，否則為產品預設冠軍）；僅對索引 1 起的剩余個體按上表約 10% / 40% / 50% 計數分配（相對剩余個體數四舍五入）；無 DB 精英時索引 1 起全部為 `RandomChromosome`。


### 2.2 並發適應度評估

評估采用固定大小 worker pool 並發執行：

$$Workers = \min(NumCPU,\; PopulationSize)$$

每個 worker 獨占一份 Adapter 執行個體（回測上下文），無共享可變狀態。所有 worker 並發處理任務佇列，等所有任務完成後進入下一代。


### 2.3 選擇（錦標賽 Tournament）

每次選擇從種群中亂數抽 `TournamentSize`（預設 3）個個體，取適應度最高者為父代。適應度為 `fatalFitnessScore`（災難性極小值）的個體在錦標賽中自然被淘汰（無需顯式排除，因 fatal 個體分數極低）。


### 2.4 交叉（Uniform Crossover）

對每個基因維度以 0.5 機率獨立地從兩個父代中選取，生成子代。引擎不關心字段語義，由策略的 `Clamp` 邏輯在交叉後修復結構約束。

當前實現：`uniformCrossoverWithMode` 對所有維度獨立 50% 交換，結構約束修復在交叉後呼叫 `repairGeneWithMode`。宏觀/微觀塊正交交叉為[規劃，未實現]。


### 2.5 變異（加性高斯）

每個基因維度以獨立 Bernoulli 機率 `MutationProbability`（預設 0.15）決定是否變異；變異量為 `NormFloat64() × GeneStep × MutationScale`（預設 Scale=1.0）。變異後呼叫 Clamp 修復越界值。
...
策略提供 Fingerprint（指紋）語義（對基因向量按精度 1e-6 哈希），引擎在 Epoch 內檢測重復個體，命中快取則復用評估結果，跳過重復回測。預期命中率約 15–30%（來自精英保留產生的重復）。
...
`SimulateGhostDCA` 以種子資本買入標的資產（如滬深300ETF或黃金ETF），然後每自然月月初注入月度預算用全部可用 CNY 加碼買入，作為被動 DCA 基準。策略個體必須跑贏此基準（Alpha > 0）才算有效。
...
策略的 ROI 采用"Modified Dietz 收益率"思路，剔除注資跳變對 NAV 的影響（注資時刻的權益變化不計入收益）。NAV 最大回撤基於凈資產曲線計算。
...
**4.4 Nested Replay（嵌套重放）：** 一次全樣本回測 + 各窗口 EvalStart checkpoint 差分，將 4 次獨立回測合並為 1 次，減少 75% 計算量。[規劃，未實現] **注意：此最佳化與 4.1 級聯短路存在根本沖突**——Nested Replay 必須先跑完整 10y 序列才能提取子窗口結果，無法在 6m fatal 時提前退出；而早期代 fatal 率通常 40–60%，10y bar 量約為 6m 的 10 倍，實施後反而劣化約 4×。僅在 fatal 率極低（< 10%）且短路收益可忽略時才有凈收益，當前不宜實現。

**4.5 前綴和 O(1) 滑窗統計 + EMA 增量化：** 減少每次適應度評估中的重復統計計算。[規劃，未實現]

**4.6 蒙特卡洛非同步化：** Epoch 驗證階段的 MC 采樣並行化，主線程等待結果。[規劃，未實現]
...
**5.1 全樣本復核：** 對冠軍在全樣本 1m 精度資料上重跑一次，確保 Adapter 內嵌的指標預熱不影響結果。

**5.2 蒙特卡洛風險評估：** 對冠軍的成交對數收益序列執行 `RunMonteCarlo`，輸出：
- 破產機率（Ruin Probability）
...
參數：
- `pop_size`：種群大小，預設 300，范圍 [10, 500]
- `max_generations`：最大代數，預設 25，范圍 [5, 50]
...
進化引擎透過此介面與具體策略完全解耦，`engine.go` 不含任何策略特有的染色體字段名或類型引用。
...
`EvaluablePlan` 在 Epoch 啟動時由 `buildEvaluablePlan` 建置，在整個世代內不可變，傳遞給所有 `Evaluate` 呼叫：

```
EvaluablePlan
├── Symbol, TemplateName         // 標的程式碼與模板標識
...
DCA 基線在 Epoch 啟動時一次性計算，代內各染色體評估直接讀取，不重復運行 DCA 回測。

### 8.3 引擎對染色體內部字段不可見原則

`engine.go` 透過八個動詞操作 `Gene`，對染色體字段名完全不可見。新增策略（如滬深300ETF、黃金ETF 或其他自定義 A 股/黃金策略）只需實現 `EvolvableStrategy` 介面，無需修改引擎程式碼。
