...
全部規劃來源皆為免費公開資料，無 API key、無授權費。下表端點已於 2026-06-13 以 `go run ./cmd/verify-marketdata` 實測可連線、可解析 JSON，並以該指令作為後續驗證證據。該指令預設每次官方 API 請求間隔 4 秒；正式 worker 也使用每來源全域 rate limiter，避免短時間連打。
