...
全部規劃來源皆為免費公開資料，無 API key、無授權費。下表端點是依公開資訊整理的計畫值，**M3 動工第一件事是逐一實測路徑、格式與限流行為，驗證結果（含確切 URL 與欄位對應）回填本表**。

| Dataset | 主來源 | 備援 / 輔助比對 | 備註 |
|---|---|---|---|
| 上市日終行情（當日全市場） | TWSE OpenAPI `exchangeReport/STOCK_DAY_ALL` | FinMind（非官方，僅輔助） | 僅當日快照，不可回補 |
| 上市日終行情（歷史回補） | TWSE 個股日成交 `exchangeReport/STOCK_DAY`（按月、逐標的） | FinMind | 限流敏感，走防鎖規則 |
| 上櫃日終行情（當日） | TPEx OpenAPI 上櫃收盤行情 | FinMind | 僅當日快照 |
| 上櫃日終行情（歷史回補） | TPEx 個股成交資訊查詢 | FinMind | 端點待驗證 |
| 除權息／公司行動 | TWSE `exchangeReport/TWT49U`（除權息計算結果）+ TPEx 對應表 | MOPS 公告、FinMind | 可事後回補；MOPS 解析成本高，列輔助 |
| 減資、現增等公司行動細節 | MOPS 公告 | 手動補登 | 頻率低，允許人工 |
| 交易日曆 | TWSE OpenAPI `holidaySchedule` | 手動維護年度假日表 | 每年抓一次整年 |
