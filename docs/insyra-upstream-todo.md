...
| 資料 I/O | 券商對帳 CSV 匯入 | ⚠️ v0.3.0 型別推斷不適用 | **維持自寫**：實測 `ReadCSV_String` 會把代號 `0050`→int64 `50`（開頭 0 遺失）、金額讀成 float64、空值→NaN，會損毀財務資料；且無關閉推斷的選項。已開 [insyra#188](https://github.com/HazelnutParadise/insyra/issues/188) 請求「no-inference / 讀為原始字串」模式，上游支援後再評估 |
| 資料 I/O | 市場資料轉換/報表匯出 | ✅（DataTable/CCL/ToCSV/Excel） | 未來分析/報表功能以 Insyra 為預設 |
...
| ⏳ 待上游 | [insyra#188](https://github.com/HazelnutParadise/insyra/issues/188)（no-inference 讀取） | `internal/reconcile/csv.go` | 維持自寫（encoding/csv + decimal）；Insyra 型別推斷會損毀代號與金額 |

## upstream issue 與採用結果

2026-06-14 提交 4 則 issue，Insyra 於 **v0.3.0（2026-07-18）** 全部實作並關閉：

1. [insyra#175](https://github.com/HazelnutParadise/insyra/issues/175) → `quant.SharpeRatio` / `MaxDrawdown` / `AnnualizedReturn`（v0.3.0 已實作，已採用）
2. [insyra#176](https://github.com/HazelnutParadise/insyra/issues/176) → `stats.NormCDF` / `stats.NormPPF`（v0.3.0 轉公開，已採用）
3. [insyra#177](https://github.com/HazelnutParadise/insyra/issues/177) → `quant.PBO` / `quant.DeflatedSharpeRatio` / `ProbabilisticSharpeRatio`（v0.3.0 已實作，已採用）
4. [insyra#178](https://github.com/HazelnutParadise/insyra/issues/178) → `quant.WalkForward`（v0.3.0 已實作，統計核心已採用）
5. [insyra#188](https://github.com/HazelnutParadise/insyra/issues/188) → 請求 `ReadCSV` 提供「關閉型別推斷 / 讀為原始字串」選項（**待上游**；在此之前券商 CSV 維持自寫）
